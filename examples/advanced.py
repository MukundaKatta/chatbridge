...
def main():
    instance = Chatbridge(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced chatbridge Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.connect_platform(input=inp, index=i) for i, inp in enumerate(inputs)]
    ...
    start = time.time()
    for i in range(1000):
        instance.connect_platform(iteration=i)
