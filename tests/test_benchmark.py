"""Benchmark tests for Chatbridge."""
import time, pytest
...
def test_accuracy_at_scale():
    c = Chatbridge()
    for i in range(500):
        r = c.connect_platform(i=i)
        assert r["ok"] is True
        assert r["n"] == i + 1

def test_reset_is_instant():
    c = Chatbridge()
    for _ in range(100): c.connect_platform()
    start = time.time()
    c.reset()
    elapsed = time.time() - start
    assert elapsed < 0.01
...
def test_memory_stable():
    ...
    c = Chatbridge()
    for _ in range(1000): c.connect_platform(data="x" * 100)
    c.reset()
    assert c.get_stats()["ops"] == 0
